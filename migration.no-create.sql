create database itisgram if not exists;
...
ALTER TABLE dialogues ADD COLUMN IF NOT EXISTS group_code VARCHAR(50) NULL;
ALTER TABLE dialogues ADD COLUMN IF NOT EXISTS avatar VARCHAR(255) NULL;

UPDATE dialogues SET group_code = COALESCE(group_code, title) WHERE type = 'group' AND group_code IS NULL;
...
BEGIN
    FOREACH grp IN ARRAY grp_list LOOP
        ...
        IF d_id IS NULL THEN
            ...
            VALUES ('group', grp, grp, (SELECT id FROM users WHERE role = 'admin' LIMIT 1), NOW(), NOW())
